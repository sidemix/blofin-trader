...
# - Detects position OPEN/CLOSE from BloFin WS 'positions'
# - Notion: create page on OPEN, update same page on CLOSE
# - Discord: lightweight alerts (optional)
...
# Property names (override via ENV if your DB uses different labels)
        ...
        while True:
            ...
            try:
                ...
                if r.status_code == 429:
                    try: retry_after = float(r.json().get("retry_after", 2.0))
                    except Exception: retry_after = 2.0
...
async def notion_create_open_page(client: httpx.AsyncClient, key: OpenKey, row: dict) -> str:
    ...
    # Use a stable Trade ID; combine symbol-side with first-seen millis from updateTime if present
...
async def notion_update_close_page(client: httpx.AsyncClient, page_id: str, key: OpenKey, row: dict):
    ...
    # Best-effort values from last snapshot
    ...
    r = await client.patch(f"https://api.notion.com/v1/pages/{page_id}", headers=N_HEADERS, json={"properties": props})
    r.raise_for_status()
                ...
                async with websockets.connect(WS_URL, ping_interval=20, ping_timeout=20, max_size=5_000_000) as ws:
                    ...
                    async for raw in ws:
                        ...
                        if not first_snapshot:
                            ...
                            for key in added:
                                symbol, direction = key
                                # Upsert: find existing open page or create a new one
                                ...
                                await send_discord(f"🟢 New trade: **{symbol}** {direction} @ avg {fnum(LATEST_ROW[key].get('entryPrice') or LATEST_ROW[key].get('avgPrice') or LATEST_ROW[key].get('averagePrice'), 6)}")

                            # --- HANDLE CLOSED TRADES ---
                            for key in removed:
                                # Use last cached snapshot for exit numbers if we still have it
                                last_row = LATEST_ROW.get(key, {})
                                page_id = OPEN_PAGE_IDS.get(key)
                                if not page_id:
                                    # best-effort: try to find it by query
